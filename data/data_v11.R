...
alldata$age <- ifelse(!is.na(alldata$age) & alldata$age > 1000 , 2014 - alldata$age, alldata$age)
alldata$age[is.na(alldata$age)] <- median(alldata$age[which(alldata$age < 100)], na.rm = TRUE)
alldata$age[alldata$age < 14] <- -1
alldata$age[alldata$age > 100] <- -1
...
sessions <- sessions %>% group_by(uid) %>% mutate(avgSecsElapsed = totalSecsElapsed/distinctActions)
...
alldata <- merge(alldata, sessions[, c("user_id", "sessionLength", "totalSecsElapsed", "distinctActions", "distinctActionTypes", "avgSecsElapsed")], by.x = "id", by.y = "user_id", all.x = TRUE)
...
alldata$cons <- paste(alldata$consActionDet, alldata$consActionType, sep = " ")
...
save(trainD, file = "./data_V11/trainD.RData")
save(testD, file  = "./data_V11/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V11/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V11/test_user_ids.csv")
tID <- data.frame(tid = tid)
write_csv(tID, "./data_V11/tid.csv")
