...
alldata$dac_week <- week(alldata$date_account_created)
...
catCols <- c("gender", "signup_method", "language", "affiliate_channel", "affiliate_provider", "first_affiliate_tracked", "signup_app", "first_device_type",
             "first_browser")
catDF <- alldata[, catCols]
catDF[is.na(catDF)] <- "unknown"
...
reduced_dtm <- removeSparseTerms(dtm, sparse = 0.998)
...
denseFeatures <- names(alldata)[!names(alldata) %in% c("id", "date_account_created", "timestamp_first_active", "country_destination", "trainFlag",
                                                       "fac_date", "sessionFlag", "consActionDet", "consAction", "cons", "fac_day", "fac_month",
                                                       "fac_year")]
...
save(trainD, file = "./data_V6/trainD.RData")
save(testD, file  = "./data_V6/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V6/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V6/test_user_ids.csv")
