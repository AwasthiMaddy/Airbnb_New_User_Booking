...
alldata$cons <- paste(alldata$consAction, alldata$consActionDet, sep = " ")
...
denseFeatures <- names(alldata)[!names(alldata) %in% c("id", "date_account_created", "timestamp_first_active", "country_destination", "trainFlag",
                                                       "fac_date", "sessionFlag", "consActionDet", "consAction", "cons")]
...
save(trainD, file = "./data_V4/trainD.RData")
save(testD, file  = "./data_V4/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V4/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V4/test_user_ids.csv")
