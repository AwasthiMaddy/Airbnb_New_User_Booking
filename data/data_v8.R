...
reduced_dtm <- removeSparseTerms(dtm, sparse = 0.9985)
...
denseFeatures <- names(alldata)[!names(alldata) %in% c("id", "date_account_created", "timestamp_first_active", "country_destination", "trainFlag",
                                                       "fac_date", "consActionDet", "consAction", "cons", "fac_day", "fac_month",
                                                       "fac_year", "sessionFlag")]
...
# save files to disk
save(trainD, file = "./data_V8/trainD.RData")
save(testD, file  = "./data_V8/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V8/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V8/test_user_ids.csv")
tID <- data.frame(tid = tid)
write_csv(tID, "./data_V8/tid.csv")
