...
reduced_dtm <- removeSparseTerms(dtm, sparse = 0.999)
...
save(trainD, file = "./data_V5/trainD.RData")
save(testD, file  = "./data_V5/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V5/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V5/test_user_ids.csv")
