...
alldata$gender[alldata$gender == "-unknown-"] <- NA
alldata$first_device_type[alldata$first_device_type == "Other/Unknown"] <- NA
alldata$first_browser[alldata$first_browser == "-unknown-"] <- NA
alldata$na_count <- apply(alldata[, 5:15], 1, function(x) sum(is.na(x)))
...
alldata$diffdate1 <- as.integer(alldata$date_account_created - alldata$fac_date)
...
engageL2 <- c("message_post", "message_thread", "send_message", "lookup_message_thread", "message_inbox", "account_transaction_history", "previous_trips", "popular_wishlists",
              "toggle_starred_thread", "pending", "share", "read_policy_click")

login <- c("login_page", "login", "login_modal", "oauth_login", "signup_login_page", "oauth_response")
...
query <- c("similar_listings", "manage_listing", "create_listing", "your_listings", "listing_reviews_page", "update_listing", "update_listing_description", "listing_reviews",
           "user_listings", "listing_recommendations", "view_listing", "listing_descriptions", "click_reviews", "move_map", "photos")

translate <- c("translate_listing_reviews" , "translations")
...
pre_book <- c("guest_itinerary", "host_guarantee", "guest_receipt", "place_worth", "calculate_worth", "user_tax_forms","change_trip_characteristics", "view_security_checks",
              "view_resolutions", "change_availability", "move_map", "click_amenities", "trip_availability", "unavailable_dates", "click_about_host", "host_respond_page")
...
book <- c("booking", "book_it", "instant_book", "request_to_book", "complete_booking", "apply_coupon", "apply_coupon_error", "apply_coupon_click", "coupon_field_focus", 
          "coupon_code_click", "apply_coupon_click_success", "at_checkpoint")

post_book <- c("post_checkout_action", "contact_host", "change_contact_host_dates", "host_home", "host_respond",  "message_to_host_focus", 
               ...
               "create_alteration_request", "your_trips", "alteration_field", "host_respond_page", "host_standard_suspension")
                                                                                                   ...
                                                                                                   ifelse(sessions$action_detail %in% pre_book, "Pre_Booking",
                                                                                                          ifelse(sessions$action_detail %in% book, "Booking",
                                                                                                                 ifelse(sessions$action_detail %in% post_book, "Post_Booking",
                                                                                                                        ifelse(sessions$action_detail %in% translate, "Translate",
                                                                                                                               "Other"))))))))))))))
...
sessions <- sessions[!is.na(sessions$action),]
...
alldata <- alldata[order(-alldata$trainFlag),] # sort the data in train ---> test manner for easy splitting
...
cp <- Corpus(VectorSource(alldata$consAction))
BigramTokenizer <- function(x) NGramTokenizer(x, Weka_control(min = 2, max = 2))
dtm <- DocumentTermMatrix(cp, control = list(tokenize = BigramTokenizer,
                                             weighting = weightTfIdf))

reduced_dtm <- removeSparseTerms(dtm, sparse = 0.99)
reduced_dtm_matrix <-   sparseMatrix(i=reduced_dtm$i, j=reduced_dtm$j, x=reduced_dtm$v,
                                     dims=c(reduced_dtm$nrow, reduced_dtm$ncol))


cp2 <- Corpus(VectorSource(alldata$cons))
dtm2 <- DocumentTermMatrix(cp2, control = list(weighting = weightTfIdf))
reduced_dtm2 <- removeSparseTerms(dtm2, sparse = 0.9985)
dtm_matrix2 <- sparseMatrix(i=reduced_dtm2$i, j=reduced_dtm2$j, x=reduced_dtm2$v,
                            dims=c(reduced_dtm2$nrow, reduced_dtm2$ncol))
...
trainD <- cBind(trainD, dtm_matrix2[1:trainC,])
testD <- cBind(Matrix(as.matrix(alldata[, denseFeatures][(trainC+1) : testC,]), sparse = TRUE), reduced_dtm_matrix[(trainC+1): testC,])
testD <- cBind(testD, dtm_matrix2[(trainC+1): testC,])

save(trainD, file = "./data_V9/trainD.RData")
save(testD, file  = "./data_V9/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V9/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V9/test_user_ids.csv")
tID <- data.frame(tid = tid)
write_csv(tID, "./data_V9/tid.csv")
