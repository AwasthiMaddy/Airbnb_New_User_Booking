...
save(trainD, file = "./data_V7/trainD.RData")
save(testD, file  = "./data_V7/testD.RData")
target <- target[1:trainC]
labels <- data.frame(country_destination = target)
write_csv(labels, "./data_V7/labels.csv")
ids <- data.frame(id = testID)
write_csv(ids, "./data_V7/test_user_ids.csv")
tID <- data.frame(tid = tid)
write_csv(tID, "./data_V7/tid.csv")
